 ...
 */
Player.prototype.takeItem = function(item){//Private variable
  if(3 > this.getPack().length){//there is room to add more items to pack
    this.getPack().push(item);
    console.log("EJL adds potion to pack");
    return true;
  } else {//the pack is full
    console.log("The pack is full, can't store");
    return false;
  }
};
...
Player.prototype.equip = function(itemToEquip){
  ...
  if(itemToEquip instanceof Weapon){
      //console.log(itemToEquip, "is a weapon");
    if(pack.indexOf(itemToEquip) > -1){//check to see if item is in pack
      if(this.equipped !== false){//checking to see if a weapon is currently equipped
        pack.splice(pack.indexOf(itemToEquip),1,this.equipped);
        this.equipped = itemToEquip;
      } else {
        pack.splice(pack.indexOf(itemToEquip),1);//remove item from equip from pack
        this.equipped = itemToEquip;
      }
    }
  }
...
var pack = this.getPack();
  if(itemToEat instanceof Food){
      //console.log(itemToEquip, "is a weapon");
    if(pack.indexOf(itemToEat) > -1){//check to see if item is in pack
